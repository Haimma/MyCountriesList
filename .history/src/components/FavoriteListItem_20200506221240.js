...
class FavoriteListItem extends Component {
    ...
    onUpdatePress() {
        console.log(this.props.country.item.name);
        this.props.CountryUpdate({name: this.props.country.item.name, favorite: this.props.countriesLists.favorite });
    }

    renderUpdateButton({country}) {
        if (this.props.countriesLists.favorite[country.name]) {
            return (
...
export default connect(mapStateToProps, {CountryUpdate})(FavoriteListItem);
